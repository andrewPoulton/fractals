...
function draw(x, y, z, type){
    ...
    for (let i = 0; i < imageData.data.length; i += settings.stride) {
		...
		const value = renderingFunction(x1 + (dx * pixelSize), y1 + (dy * pixelSize), C);
...
renderingFunctions.julia = (real,imag, C) => {
	let zr = real;
	let zi = imag;
	let iterations = 0;
...
renderingFunctions.custom = (real,imag, C) => {
	let zr = real;
	let zi = imag;
	let iterations = 0;

	while (iterations < settings.cust_it) {
		const zr_next = zr * zr - zi * zi + C.R;
		const zi_next = 2 * zi * zr + C.I;
		zr = zr_next;
		zi = zi_next;
		if ( zr > 4 ) return iterations;
		if ( zi > 4 ) return iterations;
...
renderingFunctions.mandlebrot = (real, imag, C) => {
	let zRe = 0;
	let zIm = 0;
...
renderingFunctions.ship = (real, imag, C) => {
	let zRe = 0;
	let zIm = 0;
